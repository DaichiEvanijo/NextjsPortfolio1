  ...
  return (
    <section className="space-y-5">
      <div className="flex flex-col text-xl text-center">
        Here is the list of languages and topics that I have used at my work and
        learned until now
      </div>
      <ol
        ref={ref}
        className={`p-4 bg-gradient-to-r from-blue-100 via-yellow-100 to-blue-100 rounded-2xl shadow-xl ${isLoaded[0] ? "animate-fromRight" : "opacity-0"}`}
      >
        {AccordionData.map((data, index) => {
          return (
            <li key={index} className="border border-slate-200 rounded-xl p-2">
              <div
                className="flex justify-between items-center cursor-pointer transition duration-500 ease-in-out hover:opacity-30 text-xl"

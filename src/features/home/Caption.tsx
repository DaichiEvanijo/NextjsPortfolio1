...
const Caption = () => {
  ...
  const isLoaded = useIntersectionObserver({
    options: [{ threshold: 0.2 }, { threshold: 0.2 }],
    refs: [ref1, ref2],
  });
  ...
  return (
    <>
      ...
      <h2 ref={ref1} className={`text-3xl text-center bg-gradient-to-r from-slate-400 via-yellow-200 to-yellow-500  text-transparent bg-clip-text hidden md:flex ${isLoaded[0] ? "animate-fromBelow":null}`}>
        Hello !! My name is Daichi Koyanagi
      </h2>
      <p ref={ref2} className={`text-center text-2xl ${isLoaded[1] ? "animate-fromBelow":null}`}>
